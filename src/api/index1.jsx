...
export const getMessageList = async ({ labelIds, maxResults, q, pageToken }) => {
  ...
  const mess = await getMessage(flattenedMessages.messages[0].id);
  console.log(mess.hui)
  // console.log(getBody(mess.body, "text/plain"))

  // const request = new Request('http://192.168.1.23:8000/classify/?text='.concat(mess.toString()))
  // fetch(request)
  // .then(response => {
  ...
  // let response = await fetch('http://192.168.1.23:8000/dates/?text='+mess.hui.toString())
...
export const getMessage = async(messageId) => {
  ...
  // console.log(getBody(result.payload, "text/plain"));
  const hui = getBody(result.payload, "text/plain")
  return {
    hui,
    body,
